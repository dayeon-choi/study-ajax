...
function handleRefresh() {
  for (
    ...
    var url =
      "http://openAPI.seoul.go.kr:8088/486e485945646179313036596c477073/json/GetParkInfo/" +
      i +
      "/" +
      j;
...
function updatePark(parks) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    if (addr != park.ADDR && km <= 1) {
      //주소가 있고 반경 1KM 이내에 주차장 마커표시, 산관논
      //
      ...
      if (whatrun == "weekday" && park.WEEKEND_BEGIN_TIME == "0000") {
        // 평일 이면서 주말운영시작시간=='0000'  => 주말 운영 안함(평일만 운영함)
...
function addMarker(
  ...
  if (pay == what) {
    ...
    if (run == "0000") {
      //주말운영시간이 == 0000    => 주말운영은 안한다는 의미(평일운영)
      if (tel == "") {
        //전화번호 없는경우와 있는경우
        var content =
          "<div style='width:100%; height:100%; padding:5px; font-size:0.8em;'>주차장: " +
          name +
          "(" +
          what +
          ")" +
      ...
      // 주말운영시간이 != 0000    => 평일주말운영한다는 의미(항시운영)
      if (tel == "") {
        //전화번호 없는경우와 있는경우
        var content =
          "<div style='width:100%; height:100%; padding:5px; font-size:0.8em;'>주차장: " +
