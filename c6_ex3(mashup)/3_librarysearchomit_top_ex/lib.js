...
function handleRefresh() {
  for (
    ...
    var url =
      "http://openAPI.seoul.go.kr:8088/486e485945646179313036596c477073/json/SeoulPublicLibraryInfo/" +
      i +
      "/" +
      j;
  ...
  for (var i = 0; i < librarys.length; i++) {
    ...
    if (lib.CODE_VALUE == gus) {
      ...
      div.innerHTML +=
        ...
        " onclick=\"window.open('http://www.google.co.kr/maps/search/" +
        lib.LBRRY_NAME +
        " ')\"/>";
...
function addMarker(
  ...
  var content =
    "<div style='width:100%; height:100%; padding:5px; font-size:0.8em;'>" +
    "도서관 : " +
    "<b>" +
    name +
    "</b>" +
...
function go_top(orix, oriy, desx, desy) {
  ...
  if (
    Math.floor(Math.abs(startx - orix)) < 1 &&
    Math.floor(Math.abs(starty - oriy)) < 1
  ) {
    clearTimeout(Timer); //타이머 해제
    window.scroll(orix, oriy); //올라가기
  } else if (posX != orix || posY != oriy) {
    Timer = setTimeout(
      "go_top(" + orix + "," + oriy + "," + desx + "," + desy + ")",
      15
    ); //타이머설정
  } else {
    clearTimeout(Timer); //타이머 해제
