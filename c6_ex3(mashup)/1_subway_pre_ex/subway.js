...
function getMyLocation() {
  if (navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(showMap, displayError, {
...
function showMap(position) {
  ...
  request.onload = function () {
    if (request.status == 200) {
      ...
      LL = new google.maps.LatLng(
        subways[memory].XPOINT,
        subways[memory].YPOINT
      );
      /* 가장 가까운역의 x, y 좌표 저장 */
...
function handleRefresh() {
  console.log("here");
  var url =
    "http://openapi.seoul.go.kr:8088/486e485945646179313036596c477073/json/SearchArrivalInfoByIDService/1/2/" +
    test +
    "/1/1/";
...
function updateTraffic(subways) {
  ...
  if (time == 1) {
    ...
    for (var i = 0; i < traffic.length; i++) {
      content1 +=
        "상행 도착 시간 : " +
        traffic[i].ARRIVETIME +
        "<br>상행 종점 : " +
        traffic[i].SUBWAYNAME +
        "<br><br>";
